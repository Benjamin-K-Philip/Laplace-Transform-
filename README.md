...
The goal is to take a frequency domain function $F(p)$ and return it to the time domain $f(x)$
   ...
   2. **Numerator Matching:** Adjusting the numerator to match the standard forms of Sine or Cosine. If **$(p + a)^2$** in the denominator, the numerator must also be expressed in terms of $(p + a)$ to allow for an Inverse Shift.
